...
class DatabaseCmds(object):
  # ============================== MESSAGES TABLE ==============================
    ...
    LOG_MSG=                    "SELECT * FROM logMemberMessage(CAST($1 AS BIGINT), CAST($2 AS BIGINT), CAST($3 AS BIGINT), CAST($4 AS BIGINT), $5)"
    GET_MSGS_AFTER=             "SELECT * FROM public.messages WHERE timestamp < $1"
    GET_MEMBER_MSGS_AFTER=      "SELECT * FROM public.messages WHERE auth_id = CAST($1 AS BIGINT) AND timestamp > $2"
                                #bigger date first
    GET_MEMBER_MSGS_BETWEEN=    "SELECT * FROM public.messages WHERE auth_id = CAST($1 AS BIGINT) AND timestamp < $2 AND timestamp > $3"
  ...
  # ============================== GALLARY MESSAGES TABLE ==============================
    ...
    ADD_GALL_MSG=""" 
        INSERT INTO public.gallery_messages(
            msg_id, ch_id, guild_id, auth_id, timestamp
            )
        VALUES(
            CAST($1 AS BIGINT), CAST($2 AS BIGINT), CAST($3 AS BIGINT), CAST($4 AS BIGINT), $5
    ...
    GET_GALL_MSG_AFTER=         "SELECT * FROM public.gallery_messages WHERE timestamp > $1"
    GET_GALL_MSG_BEFORE=        "SELECT * FROM public.gallery_messages WHERE timestamp < $1"
    GET_GALL_MSG_MEM_AFTER=     "SELECT * FROM public.gallery_messages WHERE auth_id = CAST($1 AS BIGINT) AND timestamp > $2"
    ...
    DEL_GALL_MSGS_FROM_CH=      "DELETE FROM public.gallery_messages WHERE ch_id = CAST($1 AS BIGINT) AND guild_id = CAST($2 AS BIGINT);"
  ...
  # ============================== MEMBERS TABLE ==============================
    ...
    REMOVE_MEMBER_FUNC=         "UPDATE public.members SET ishere = FALSE WHERE user_id = $1;"
    GET_ALL_MEMBERS_RAW=        "SELECT FROM public.members;"
    GET_ALL_MEMBERS=            "SELECT user_id, ishere FROM public.members;"
    GET_MEMBER_ID=              "SELECT * FROM public.members WHERE user_id = CAST($1 AS BIGINT) LIMIT 1;"
    GET_MEMBER_LEADERBOARD =    "SELECT * FROM public.members WHERE ishere = TRUE ORDER BY nummsgs DESC LIMIT 10;"
    MEMBER_TABLE_EMPTY=         "SELECT * FROM public.members ORDER BY num ASC LIMIT 1;"
  ...
  # ============================== INVITE TABLE ==============================
    ...
    ADD_INVITES=                "SELECT updatestoredinvites($1);"
    GET_INVITE_DATA=            "SELECT * FROM invites;"
  ...
  # ============================== ARTIST INFO TABLE ==============================
    ...
    GET_ALL_ARTIST_INFO=            "SELECT * FROM public.artist_info;"
    UPDATE_ARTIST_INFO="""
        ...
        ON CONFLICT (user_id)
            ...
            SET 
                info = $2 
            WHERE user_id = CAST($1 AS BIGINT);
        """
  ...
  # ============================== WELCOME MESSAGE TABLE ==============================
    ...
    ADD_WEL_MSG=""" 
        INSERT INTO public.welcome_msg (
    ...
    REM_MEM_WEL_MSG=            "DELETE FROM public.welcome_msg WHERE user_id = $1"
    GET_MEM_WEL_MSG=            "SELECT * FROM public.welcome_msg WHERE user_id = $1"
    GET_ALL_WEL_MSG=            "SELECT * FROM public.welcome_msg"
    EXISTS_WEL_MSG_TABLE=       "SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE upper(table_name) = 'WELCOME_MSG');"

    
  # ============================== REACTION MESSAGE TABLE ============================== to be deleted
  ...
  # ============================== GIVEAWAY WINNERS TABLE ==============================
    ...
    ADD_GVWY_PRE_WINS= """
        ...
        ON CONFLICT (user_id)
            ...
            UPDATE
            SET 
                num_wins = gvwy_previous_wins.num_wins + 1,
                last_win = $2
  ...
  # ============================== DM FEEDBACK TABLE ==============================
    ...
    GET_MEM_DM_FEEDBACK="""
        SELECT * FROM dm_feedback 
        WHERE 
            sent_msg_id = CAST($1 AS BIGINT) AND sent_guild_id = CAST($2 AS BIGINT)
        LIMIT 1
        """

    GET_MEM_CH_DM_FEEDBACK="""
        SELECT * FROM dm_feedback 
        WHERE 
            sent_msg_id = CAST($1 AS BIGINT) AND sent_chl_id = CAST($2 AS BIGINT) AND sent_guild_id = CAST($3 AS BIGINT)
        LIMIT 1
        """
  ...
  # ============================== FUNCTIONS ==============================
    ...
    CREATE_FUNC_ARTIST_INFO =  """  
        ...
        IF NOT EXISTS(SELECT * FROM pg_proc WHERE prorettype <> 0 AND proname = 'updateartistinfo') THEN 
            ...
            COMMENT ON function IS 'This function is kinda unnessesary but it works so pfffff. Basically it either inserts or updates artist info data for the artsts.';
        END IF;
        END
        $do$
        """
    ...
    CREATE_FUNC_LOG_MSG = """
        ...
        $do$
        BEGIN
        IF NOT EXISTS(SELECT * FROM pg_proc WHERE prorettype <> 0 AND proname = 'logmembermessage') THEN 
            CREATE FUNCTION logMemberMessage(nmsg_id BIGINT, nch_id BIGINT, nguild_id BIGINT, nauth_id BIGINT, ntimestamp TIMESTAMP) 
            ...
            DECLARE
                new_level SMALLINT;
                new_nummsgs BIGINT;
                old_level SMALLINT := (SELECT level FROM public.members WHERE user_id = nauth_id);
            ...
            BEGIN
                ...
                VALUES( 
                    CAST(nmsg_id AS BIGINT), CAST(nch_id AS BIGINT), CAST(nguild_id AS BIGINT), CAST(nauth_id AS BIGINT), ntimestamp
                    )
                ON CONFLICT (msg_id)
                    DO NOTHING;
                ...
                new_nummsgs = (SELECT nummsgs FROM public.members WHERE user_id = nauth_id);
                ...
                return QUERY SELECT leveled_up, new_level, old_level;
            END;
            '
            LANGUAGE plpgsql
            COST 400;

            COMMENT ON FUNCTION logmembermessage IS 'This function both inserts a new message into the messages table and works out if the member has leveled up or not. Returns a table which contains a bool if the member has leveled up or not, plus their new and old level.';
