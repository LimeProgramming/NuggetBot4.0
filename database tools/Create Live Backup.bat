...
SET NEW_DBNAME=%DBNAME%-backup %datetime%

bin\createdb --host="%PGCHOST%" --port=%PGCPORT% --username="%PGCLOGIN%" "%NEW_DBNAME%"

REM Allow pg_dump to print it's output, useful for errors.
2>logs\%DBNAME%_%datetime%_RLog.txt (
    bin\pg_restore --host="%PGCHOST%" --port=%PGCPORT% --username="%PGCLOGIN%" --dbname="%NEW_DBNAME%" -v "%BACKUP_FILE%"
)
